 ...
 *  Compilation: gcc -o Conv_lbs_kg Conv_lbs_kg.c to compile
 *  Execution:
 *      server: ./Conv_lbs_kg portnum
 *
 **************************************************************************/
#include <stdio.h>
#include <stdlib.h>
...
process(int sock)
{
    ...
    double a;
    char* error;
    if(strcmp(arg_1,"lbs")==0 && strcmp(arg_2,"kg")==0){
        //sprintf(reply, "%f\n", strtod(arg_3, NULL)*7.5);
        a=strtod(arg_3, NULL) / 2.2;
        printf(" %f\n", a);
        //reply=(char*)(&a);
        snprintf(reply, 50, "%f\n", a);
        write(sock, reply, strlen(reply));
    }
    else if(strcmp(arg_1,"kg")==0 && strcmp(arg_2,"lbs")==0){
        //sprintf(reply, "%f\n", strtod(arg_3, NULL)/7.5);
        a=strtod(arg_3, NULL) * 2.2;
        snprintf(reply, 50, "%f\n", a);
        write(sock, reply, strlen(reply));
